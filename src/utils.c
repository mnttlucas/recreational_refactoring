...
void bin_arr_to_hex_arr(uint8_t *bin_arr, char *hex_arr)
{
	...
	for(int i = 0; i < 8; i++)
	{
		...
		if(hex_value <= 9) hex_arr[i] = (char) ('0' + hex_value);
		else hex_arr[i] = (char) ('a' + (hex_value - 10));
	}
...
void bin_twos_complement(int start, int end, uint8_t *arr)
{
	...
	{
		if(found_one) arr[i] ^= 1;
		else if(arr[i] == 1) found_one = 1;
	}
}
...
{
	if(!instr->exit) printf("%s", instr->to_string);
}
...
uint8_t register_string_to_int(char *reg)
{
	...
	else
	{
		...
		else if(!strcmp(reg, "at"))	reg_int = REG_AT;
		else if(!strcmp(reg, "gp"))	reg_int = REG_GP;
		else if(!strcmp(reg, "sp"))	reg_int = REG_SP;
		else if(!strcmp(reg, "fp"))	reg_int = REG_FP;
		else if(!strcmp(reg, "ra"))	reg_int = REG_RA;
		else if(reg[0] == 'v')
		{
		...
		else if(reg[0] == 't')
		{
			...
			else if(reg[1] == '8' || reg[1] == '9') reg_int = REG_T8 + (reg_1 - 8);
		}
